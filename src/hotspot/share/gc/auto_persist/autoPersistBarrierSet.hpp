...
RETRY:
      ...
      OrderAccess::fence();
      if (base->autopersist_nvm_header() != 0) goto RETRY;
    }
...
RETRY:
      ...
      Parent::arraycopy_in_heap(src_obj, src_offset_in_bytes, src_raw,
                                ...
                                length);
      OrderAccess::fence();
      if (src_obj != NULL && src_obj->autopersist_nvm_header() != 0) goto RETRY;
      if (dst_obj != NULL && dst_obj->autopersist_nvm_header() != 0) goto RETRY;
    }
...
RETRY:
      ...
      OrderAccess::fence();
      if (src != NULL && src->autopersist_nvm_header() != 0) goto RETRY;
      if (dst != NULL && dst->autopersist_nvm_header() != 0) goto RETRY;
    }
  };
};
