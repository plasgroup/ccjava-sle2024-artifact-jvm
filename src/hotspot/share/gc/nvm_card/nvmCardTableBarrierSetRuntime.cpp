...
address NVMCardTableBarrierSetRuntime::write_nvm_field_post_entry(DecoratorSet decorators, BasicType type) {
  assert((decorators == 537142272ULL) || (decorators == 537141312ULL) || (decorators == 537273344ULL) || (decorators == 538189888ULL) || (decorators == 805577728ULL) || (decorators == 2199828833280), " unknown decorator base");
  ...
  } else if (decorators == 2199828833280ULL) {
    if (type == T_INT) return reinterpret_cast<address>(CallRuntimeBarrierSet::c1_call_runtime_atomic_add_at_in_heap<2199828833280ULL, jint>);
    else if (type == T_LONG) return reinterpret_cast<address>(CallRuntimeBarrierSet::c1_call_runtime_atomic_add_at_in_heap<2199828833280ULL, jlong>);
  }
